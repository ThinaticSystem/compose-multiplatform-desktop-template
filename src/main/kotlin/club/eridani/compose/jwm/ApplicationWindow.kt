...
open class ApplicationWindow(
    ...
    private fun updateConstrain() {
        ...
        composeScene.constraints =
            Constraints(maxWidth = window.contentRect.width, maxHeight = window.contentRect.height)
    }
    ...
    override fun accept(e: Event) {
        when (e) {
            ...
            is EventFrameSkia -> composeScene.render(
                e.surface.canvas.clear(Color.TRANSPARENT),
                System.nanoTime()
            )
            ...
            is EventWindowMove -> windowState?.position =
                WindowPosition(e.windowLeft.dp, e.windowTop.dp)
...
private fun createPlatformLayer(): Layer {
    return when (Platform.CURRENT
        ?: return runCatching { LayerGLSkia() }.getOrElse { LayerRasterSkia() }) {
