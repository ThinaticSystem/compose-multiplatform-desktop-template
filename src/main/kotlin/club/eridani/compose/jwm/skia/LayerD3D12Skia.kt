...
class LayerD3D12Skia : LayerD3D12() {
    ...
    override fun frame() {
        BackendRenderTarget.makeDirect3D(_width, _height, nextDrawableTexturePtr(), format, sampleCount, levelCount)
            .use { rt ->
                Surface.makeFromBackendRenderTarget(context!!, rt, origin, colorFormat, colorSpace, surfaceProps)
                    .use { surface ->
